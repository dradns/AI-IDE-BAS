...
>>>>>>> 80435003c3d37972a23719f787e550cc7b3b21cf
	...
	// Правильная обработка сообщений от VS Code Host и window.postMessage
	useEffect(() => {
...
<<<<<<< HEAD
				...
				if (typeof message.text === "string" && message.text) {
					...
					setError(undefined)
					noticeTimeout = window.setTimeout(() => setNotice(undefined), 5000)
				}
			} else if (message?.type === "files:error") {
				window.clearTimeout(errorTimeout)
				setError(String(message.error || "Ошибка"))
				setNotice(undefined)
				errorTimeout = window.setTimeout(() => setError(undefined), 8000)
=======
