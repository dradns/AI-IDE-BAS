...
const App = () => {
	...
	useEffect(() => {
		if (didHydrateState) {
			telemetryClient.updateTelemetryState(telemetrySetting, telemetryKey, machineId, telemetryHost)

			// Emit daily 'Extension Activated' from webview for proper Session Replay linkage
			try {
				const today = new Date().toISOString().slice(0, 10)
				const key = `extActivated_${today}`
				if (!localStorage.getItem(key)) {
					telemetryClient.capture(TelemetryEventName.EXTENSION_ACTIVATED, {
						source: "webview",
						sessionId: telemetryClient.getSessionId?.() || undefined,
						machineId,
					})
	...
	}, [telemetrySetting, telemetryKey, telemetryHost, machineId, didHydrateState])
